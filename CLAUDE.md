...
Swift compiler. A `.testTarget` works wherever full Xcode is installed, but the
plain target also runs under a Command Line Tools–only toolchain, which ships
no `xctest` binary to load `.xctest` bundles. Keep it that way: it costs
little, and it means the suite runs in the cheapest environment a contributor
might have. Unit tests cover `EspressoCore` only, substituting stub scripts for
`caffeinate` so tests never hold real power assertions.
...
- Never push without an explicit request. The "Session Completion" block above
  is generated by beads and asserts the opposite; this section takes precedence.
- Contributors without push access: open a pull request. CI runs on it, and
  release tags are cut by the maintainer.
